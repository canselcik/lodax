...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        LDWebsocket wss = new LDWebsocket("localhost", 9025);
        wss.start();
        ...
        if (secret != null && key != null && passphrase != null) {
            ...
            if (!props.containsKey("GDAX_KEY") || !props.containsKey("GDAX_PASSPHRASE") || !props.containsKey("GDAX_SECRET")) {
                if (GraphicsEnvironment.isHeadless()) {
                    logger.info("Detected JVM is running headless. Skipping configUI display. Provide GDAX_* env variables to use logged in features");
                } else {
        ...
        orderbook.enableWsBroadcast(wss);
        orderbook.sync();
        ...
        LDGUI graph = null;
        if (!GraphicsEnvironment.isHeadless()) {
            graph = new LDGUI(fixSession, orderbook, metaProvider);
        ...
        while (!wsDisconnected) {
            ...
            if (lp != null) {
                ...
                wss.broadcast(LDMsgBase.generateLTPMessage(lp, bb, ba));
            }
        }
