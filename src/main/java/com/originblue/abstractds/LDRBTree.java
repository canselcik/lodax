...
removeFixup() easier and more efficient.  Every LDRBNode instantiated has
...
course textbook and so are the leftRotate(LDRBNode x) and
rightRotate(LDRBNode y) functions.
...
our delete(LDRBNode v) function with a call to search(v.key) which will
...
leftRotateFixup(LDRBNode) and rightRotateFixup(LDRBNode) which update
these variables when a rotation occurs. This value is also updated during the
deletion of a node by the function called fixNodeData(LDRBNode, int).
...
In the program, I am checking for the case where a particular LDRBNode has
a pointer pointing to nil, since this operation is very common, I have a
function called isNil(LDRBNode), which returns a boolean value of whether
...
// Class Definitions
public class LDRBTree<T extends Comparable<T>> {

    // Root initialized to nil.
    private LDRBNode<T> nil = new LDRBNode<T>();
    private LDRBNode<T> root = nil;
    private Semaphore semaphore = new Semaphore(1);
    private Map<String, LDRBNode<T>> lookupTable = new HashMap<String, LDRBNode<T>>();
    ...
    private void leftRotate(LDRBNode<T> x) {
    ...
    }// end leftRotateFixup(LDRBNode x)
    ...
    private void rightRotate(LDRBNode<T> y) {
    ...
    }// end rightRotate(LDRBNode y)


    // @param: y, the node around which the righRotate is to be performed.
    // Updates the numLeft and numRight values affected by the rotate
    private void rightRotateFixup(LDRBNode y) {
    ...
    }// end rightRotateFixup(LDRBNode y)

    public void insert(T key, String secondaryKey) {
        try {
            semaphore.acquire();
        }
        catch (Exception e) {
        ...
        LDRBNode<T> node = new LDRBNode<T>(key);
        this.lookupTable.put(secondaryKey, node);
        insert(node);
        semaphore.release();
    }

    public LDRBNode<T> lookupBySecondaryKey(String secondaryKey) {
        return this.lookupTable.get(secondaryKey);
    }

    // @param: z, the node to be inserted into the Tree rooted at root
    // Inserts z into the appropriate position in the LDRBTree while
    // updating numLeft and numRight values.
    private void insert(LDRBNode<T> z) {
        ...
        z.left = nil;
        z.right = nil;
        z.color = LDRBNode.RED;

        // Call insertFixup(z)
        insertFixup(z);
    }// end insert(LDRBNode z)


    // @param: z, the node which was inserted and may have caused a violation
    // of the LDRBTree properties
    // Fixes up the violation of the LDRBTree properties that may have
    // been caused during insert(z)
    private void insertFixup(LDRBNode<T> z) {

        LDRBNode<T> y = nil;
        // While there is a violation of the LDRBTree properties..
        while (z.parent.color == LDRBNode.RED) {
            ...
            if (z.parent == z.parent.parent.left) {
                ...
                if (y.color == LDRBNode.RED) {
                    z.parent.color = LDRBNode.BLACK;
                    y.color = LDRBNode.BLACK;
                    z.parent.parent.color = LDRBNode.RED;
                    z = z.parent.parent;
                }
        ...
        root.color = LDRBNode.BLACK;

    }// end insertFixup(LDRBNode z)Q

    public LDRBNode<T> treeRoot() {
        return this.root;
    }

    // @param: node, a LDRBNode
    // @param: node, the node with the smallest key rooted at node
    public LDRBNode<T> treeMinimum(LDRBNode<T> node) {

        // while there is a smaller key, keep going left
        while (!isNil(node.left))
            node = node.left;
        return node;
    }// end treeMinimum(LDRBNode node)


    // @param: x, a LDRBNode whose successor we must find
    // @return: return's the node the with the next largest key
    // from x.key
    public LDRBNode<T> treeSuccessor(LDRBNode<T> x) {
        ...
        // Return successor
        return y;
    }// end treeMinimum(LDRBNode x)


    // @param: z, the LDRBNode which is to be removed from the the tree
    // Remove's z from the LDRBTree rooted at root
    public void remove(LDRBNode<T> v, String secondaryKey) {
        try {
            semaphore.acquire();
        } catch (Exception e) {
            e.printStackTrace();
        }
        this.lookupTable.remove(secondaryKey);

        LDRBNode<T> z = search(v.key);
        ...
        // LDRBTree properties so call removeFixup()
        if (y.color == LDRBNode.BLACK)
            removeFixup(x);
        semaphore.release();
    }// end remove(LDRBNode z)


    // @param: y, the LDRBNode which was actually deleted from the tree
    // @param: key, the value of the key that used to be in y
    private void fixNodeData(LDRBNode<T> x, LDRBNode<T> y) {

        // Initialize two variables which will help us traverse the tree
        LDRBNode<T> current = nil;
        LDRBNode<T> track = nil;
    ...
    // @param: x, the child of the deleted node from remove(LDRBNode v)
    // Restores the Red Black properties that may have been violated during
    // the removal of a node in remove(LDRBNode v)
    private void removeFixup(LDRBNode<T> x) {

        LDRBNode<T> w;

        // While we haven't fixed the tree completely...
        while (x != root && x.color == LDRBNode.BLACK) {
            ...
            if (x == x.parent.left) {
                ...
                if (w.color == LDRBNode.RED) {
                    w.color = LDRBNode.BLACK;
                    x.parent.color = LDRBNode.RED;
                    leftRotate(x.parent);
                    w = x.parent.right;
                }

                // Case 2, both of w's children are black
                if (w.left.color == LDRBNode.BLACK &&
                        w.right.color == LDRBNode.BLACK) {
                    w.color = LDRBNode.RED;
                    x = x.parent;
                }
                // Case 3 / Case 4
                else {
                    // Case 3, w's right child is black
                    if (w.right.color == LDRBNode.BLACK) {
                        w.left.color = LDRBNode.BLACK;
                        w.color = LDRBNode.RED;
                        rightRotate(w);
                        w = x.parent.right;
                    }
        ...
        x.color = LDRBNode.BLACK;
    }// end removeFixup(LDRBNode x)
    ...
    public LDRBNode<T> search(T key) {

        // Initialize a pointer to the root to traverse the tree
        LDRBNode<T> current = root;
    ...
    public int findNumGreater(LDRBNode<T> node, T key) {
    ...
    }// end findNumGreater(LDRBNode, int key)
    ...
    private void getGreaterThan(LDRBNode<T> node, T key,
                                List<T> list) {
        if (isNil(node)) {
            return;
    ...
    public int findNumSmaller(LDRBNode<T> node, T key) {
    ...
    }// end findNumSmaller(LDRBNode nod, int key)


    // @param: node, the LDRBNode we must check to see whether it's nil
    // @return: return's true of node is nil and false otherwise
    private boolean isNil(LDRBNode node) {

        // return appropriate value
        return node == nil;

    }// end isNil(LDRBNode node)


    // @return: return's the size of the tree
    // Return's the # of nodes including the root which the LDRBTree
    // rooted at root has.
    public int size() {
...
I chose the object LDRBNode class to have seven instance variables which are
all declared public as per the assignment specifications.  Each instance of a
LDRBNode holds a Comparable "key", which is the key of the LDRBNode.  It
...
Each instance also holds a LDRBNode pointer to the node's "parent", "left"
...
also in case we want to just create a LDRBNode and initialize its key later.
...
in the LDRBTree class as it is most referenced there, in this class
